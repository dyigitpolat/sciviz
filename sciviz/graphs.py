...
* :class:`BipartiteGraph` -- two-column graph with weighted edges between sides
                            (replaces the custom MoE RouterDiagram).
* :class:`NodeTree`      -- top-down tree with node groups acting as one page
                            and visible per-cell separators (replaces the
                            custom B+ tree element).
* :class:`Sequence`      -- UML-style sequence diagram with lifelines, message
                            arrows between participants (replaces Paxos abuse
                            of Timeline).
* :class:`FlowChart`     -- vertical chain of boxes joined by arrows; supports
                            branches and merges via simple list-of-lists syntax.
"""
...
class BipartiteGraph(Element):
    """A two-column graph with weighted edges from left nodes to right nodes.

    Generic enough to express:
      * MoE token -> expert routing
      * preference matchings
      * RPC client -> server graphs
      * any small bipartite assignment

    Parameters
    ----------
    left : list of str
    right : list of str
    edges : list of (left_idx, right_idx, weight) or (left_idx, right_idx)
        Weights default to 1.0; they control edge thickness.
    height : float
        Total drawing height (left and right columns are stretched to fit).
    column_gap : float
        Horizontal gap between the two columns of nodes.
    node_w : float
        Node chip width.
    node_h : float
        Node chip height.
    left_role, right_role : str
        Color roles for the two columns.
    color_by : str
        ``"right"`` (default) -- edges share the colour of their right endpoint
        ``"left"``           -- edges share the colour of their left endpoint
        ``"weight"``         -- edges shaded by weight using the primary palette
    """

    def __init__(self, left: Seq[str], right: Seq[str],
                 edges: Seq, *,
                 height: float = 220.0,
                 column_gap: float = 220.0,
                 node_w: float = 60.0, node_h: float = 22.0,
                 left_role: str = "blue",
                 right_role: str = "auto",
                 color_by: str = "right"):
        self.left = list(left)
        ...
        for e in edges:
            if len(e) == 2:
                norm.append((int(e[0]), int(e[1]), 1.0))
            elif len(e) == 3:
                norm.append((int(e[0]), int(e[1]), float(e[2])))
            else:
                raise ValueError(f"Edge needs (li, ri[, weight]); got {e}")
        self.edges = norm
        self.height = height
    ...
    def measure(self, theme: Theme) -> BBox:
        return BBox(2 * self.node_w + self.column_gap, self.height)

    def _y_centres(self, n: int, y: float) -> List[float]:
        if n == 1:
            return [y + self.height / 2]
        gap = (self.height - self.node_h * n) / (n - 1)
        return [y + i * (self.node_h + gap) + self.node_h / 2 for i in range(n)]

    def _node_color(self, role: str, idx: int, used: bool, theme: Theme):
        """Return (fill, stroke, text_color) for a node with given role."""
        if role == "auto":
            if not used:
                return ("#ffffff", theme.color_of("border_strong"),
                        theme.color_of("text_muted"))
            r = theme.role_for_index(idx)
            fill = theme.role(r, "fill")
            return (fill, theme._darken(fill, 0.30), theme.text_on(fill))
        if not used:
            return ("#ffffff", theme.color_of("border_strong"),
                    theme.color_of("text_muted"))
    ...
    def render(self, canvas: Canvas, x: float, y: float, theme: Theme) -> None:
        ...
        ly = self._y_centres(nL, y)
        ry = self._y_centres(nR, y)
        lx_right = x + self.node_w
        rx_left = x + self.node_w + self.column_gap

        # which right nodes are used?
        used_right = {ri for _, ri, _ in self.edges}
        used_left = {li for li, _, _ in self.edges}

        # edges first, so nodes draw on top
        for li, ri, w in self.edges:
            x1, y1 = lx_right, ly[li]
            x2, y2 = rx_left, ry[ri]
            if self.color_by == "left":
                role = self.left_role if self.left_role != "auto" else theme.role_for_index(li)
                col = theme.role(role, "fill")
            elif self.color_by == "weight":
                col = theme.color_scale(w, "blues", 0, 1)
            else:  # "right"
                role = self.right_role if self.right_role != "auto" else theme.role_for_index(ri)
                col = theme.role(role, "fill")
            canvas.line(x1, y1, x2, y2,
                       stroke=col,
                       stroke_width=max(0.6, w * 2.0),
                       opacity=0.55 + 0.45 * w)

        # left nodes
        for i, lbl in enumerate(self.left):
            cy = ly[i] - self.node_h / 2
            fill, stroke, tc = self._node_color(self.left_role, i,
                                               i in used_left, theme)
            canvas.rect(x, cy, self.node_w, self.node_h,
                       fill=fill, stroke=stroke,
                       stroke_width=theme.hairline, rx=2)
            canvas.text(x + self.node_w / 2,
                       cy + self.node_h / 2 + theme.size_px("small") * 0.33,
                       lbl, size=theme.size_px("small"),
                       fill=tc, weight="700", anchor="middle")

        # right nodes
        for i, lbl in enumerate(self.right):
            cy = ry[i] - self.node_h / 2
            fill, stroke, tc = self._node_color(self.right_role, i,
                                               i in used_right, theme)
            canvas.rect(rx_left, cy, self.node_w, self.node_h,
                       fill=fill, stroke=stroke,
                       stroke_width=theme.hairline, rx=2)
            canvas.text(rx_left + self.node_w / 2,
                       cy + self.node_h / 2 + theme.size_px("small") * 0.33,
                       lbl, size=theme.size_px("small"),
                       fill=tc, weight="700", anchor="middle")
...
# ---------------------------------------------------------------------------
# FlowChart -- a vertical chain of boxes joined by arrows, supports branches
# ---------------------------------------------------------------------------

class FlowChart(Element):
    """Vertical sequence of labelled boxes joined by arrows.

    Each step is either:
      * a string ``"label"`` -- single straight box
      * a tuple ``("label", role)`` -- coloured single box
      * a list ``[step1, step2, ...]`` -- a horizontal branch (parallel rows)

    Arrows are drawn automatically between adjacent steps; branches collapse
    back to a single trunk afterwards (a 'merge node').
    ...
    arrow_gap : float
    role : str
        Default colour role for boxes.
    """

    def __init__(self, steps: Seq, *,
                 box_w: float = 160.0, box_h: float = 28.0,
                 arrow_gap: float = 22.0,
                 role: str = "blue",
                 branch_gap: float = 28.0):
        self.steps = list(steps)
        self.box_w = box_w
        self.box_h = box_h
        self.arrow_gap = arrow_gap
        self.role = role
        self.branch_gap = branch_gap

    def _normalise(self, step):
        """Return list of (label, role) for one step row (1 entry = trunk)."""
        if isinstance(step, str):
            return [(step, self.role)]
        if isinstance(step, tuple) and len(step) == 2 and isinstance(step[0], str):
            return [(step[0], step[1] or self.role)]
        if isinstance(step, list):
            out = []
            for s in step:
                if isinstance(s, str):
                    out.append((s, self.role))
                elif isinstance(s, tuple) and len(s) == 2:
                    out.append((s[0], s[1] or self.role))
                else:
                    raise TypeError(f"Bad branch step: {s}")
            return out
        raise TypeError(f"Bad flowchart step: {step}")

    def _row_widths(self, steps):
        return [len(s) for s in steps]

    def measure(self, theme: Theme) -> BBox:
        steps = [self._normalise(s) for s in self.steps]
        max_branch = max(self._row_widths(steps), default=1)
        w = max_branch * self.box_w + (max_branch - 1) * self.branch_gap
        h = (len(steps) * self.box_h
             + (len(steps) - 1) * self.arrow_gap)
        return BBox(w, h)

    def render(self, canvas: Canvas, x: float, y: float, theme: Theme) -> None:
        steps = [self._normalise(s) for s in self.steps]
        size = self.measure(theme)
        cx_centre = x + size.w / 2
        cy = y
        prev_anchor_xs = None  # x-coords of bottom anchors of previous row
        for row in steps:
            n = len(row)
            row_w = n * self.box_w + (n - 1) * self.branch_gap
            row_x = cx_centre - row_w / 2
            top_anchors = []
            bottom_anchors = []
            # boxes
            for i, (lbl, role_name) in enumerate(row):
                bx = row_x + i * (self.box_w + self.branch_gap)
                fill = theme.role(role_name, "fill")
                stroke = theme.role(role_name, "stroke")
                tc = theme.text_on(fill)
                canvas.rect(bx, cy, self.box_w, self.box_h,
                           fill=fill, stroke=stroke,
                           stroke_width=theme.hairline, rx=2)
                canvas.text(bx + self.box_w / 2,
                           cy + self.box_h / 2 + theme.size_px("label") * 0.33,
                           lbl, size=theme.size_px("label"),
                           fill=tc, weight="600", anchor="middle")
                top_anchors.append(bx + self.box_w / 2)
                bottom_anchors.append(bx + self.box_w / 2)
            # arrows from previous bottom anchors to this row's top anchors
            if prev_anchor_xs is not None:
                color = theme.color_of("text")
                marker = canvas.define_marker(color=color, size=5,
                                              name_hint="flowArr")
                if len(prev_anchor_xs) == 1 and len(top_anchors) > 1:
                    src = prev_anchor_xs[0]
                    src_y = cy - self.arrow_gap
                    for tx in top_anchors:
                        canvas.line(src, src_y, tx, cy,
                                   stroke=color, stroke_width=theme.line,
                                   marker_end=marker)
                elif len(prev_anchor_xs) > 1 and len(top_anchors) == 1:
                    dst = top_anchors[0]
                    for sx in prev_anchor_xs:
                        canvas.line(sx, cy - self.arrow_gap, dst, cy,
                                   stroke=color, stroke_width=theme.line,
                                   marker_end=marker)
                else:
                    for sx, tx in zip(prev_anchor_xs, top_anchors):
                        canvas.line(sx, cy - self.arrow_gap, tx, cy,
                                   stroke=color, stroke_width=theme.line,
                                   marker_end=marker)
            prev_anchor_xs = bottom_anchors
            cy += self.box_h + self.arrow_gap
