...
class Panel(Element):
    ...
    def render(self, canvas: Canvas, x: float, y: float, theme: Theme) -> None:
        ...
        # A peer-matched titled panel should keep its first semantic row
        # aligned with its neighbours. Centring a short Column creates a
        # false blank header band; compact leaf visuals remain centred.
        if structural_body and surplus > theme.unit * 2.0:
            child_y = content_top
        else:
            child_y = content_top + surplus / 2
