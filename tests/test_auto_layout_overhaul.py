...
from sciviz.core import Canvas, Element, Theme
from sciviz.core import FontRegistry


def test_svg_embeds_font_face():
    svg = Diagram.for_paper(Text("glyph safe")).render()
    assert "@font-face" in svg
    assert "sciviz-" in svg
...
def test_png_auto_text_mode_outlines_text(monkeypatch, tmp_path):
    calls = {}
